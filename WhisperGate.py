...
def export_excel():
    ...
    for target in targets.values():
        ...
        ws.cell(row=row, column=3, value=cred['timestamp']).border = thin_border
        ws.cell(row=row, column=4, value=target['ip']).border = thin_border
        ws.cell(row=row, column=5, value=target['user_agent']).border = thin_border
        ws.cell(row=row, column=6, value=target['status']).border = thin_border
        mfa = 'Yes' if target['status'] in ('compromised', 'released') else 'No'
        ws.cell(row=row, column=7, value=mfa).border = thin_border
